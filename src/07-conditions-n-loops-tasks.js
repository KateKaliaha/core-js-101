...
function doRectanglesOverlap(rect1, rect2) {
  return rect1.width + rect1.left > rect2.left && rect1.left < rect2.width + rect2.left
...
function isCreditCardNumber(ccn) {
  ...
  if (digits.length % 2 === 0) {
    digits = digits.map((digit, ind) => (ind % 2 === 0 ? digit * 2 : digit));
  } else {
    digits = digits.map((digit, ind) => (ind % 2 === 1 ? digit * 2 : digit));
  }
...
function getMatrixProduct(m1, m2) {
  const rowsFirst = m1.length;
  const rowsSecond = m2.length;
  const colSecond = m2[0].length;
  const resMatrix = [];
  for (let i = 0; i < rowsFirst; i += 1) {
    resMatrix[i] = [];
  }

  for (let j = 0; j < colSecond; j += 1) {
    for (let i = 0; i < rowsFirst; i += 1) {
      let el = 0;
      for (let j1 = 0; j1 < rowsSecond; j1 += 1) {
        el += m1[i][j1] * m2[j1][j];
      }
      resMatrix[i][j] = el;
    }
